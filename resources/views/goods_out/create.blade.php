...
<div class="container">
    ...
    <form method="POST" action="{{ route('goods_out.store') }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" value="{{ $materialRequest->qty }} {{ $materialRequest->inventory->unit }}" disabled>
        </div>
        <div class="mb-3">
            <label>Quantity to Goods Out</label>
            <input type="number" name="quantity" class="form-control" value="{{ $materialRequest->qty }}" step="0.01" required>
        </div>
        <div class="mb-3">
...
<script>
    $(document).ready(function() {
        ...
        $('select[name="inventory_id"]').on('change', function() {
            ...
            $('input[name="quantity"]').next('.unit-label').text(selectedUnit || 'unit');
        });
    });
</script>
@endpush
